...
class StockService {
    ...
    return [
      ...
      },
      {
        $lookup: {
          ...
          pipeline: [priceMatchStage, { $sort: { createdAt: -1 } }, { $limit: 1 }],
          as: 'price',
        },
      },
      {
  ...
  updateStock(stockId, minQuantity, userId = null, reason = null) {
    return Stock.findById(stockId).then(stock => {
      ...
      return Stock.findByIdAndUpdate(stockId, { minQuantity }, { new: true })
        .populate('productId')
        .then(updatedStock => {
  ...
  addStock(stockId, amount, userId = null, billId = null) {
    return Stock.findById(stockId).then(stock => {
      ...
      return Stock.findByIdAndUpdate(stockId, { $inc: { quantity: amount } }, { new: true })
        .populate('productId')
        .then(updatedStock => {
  ...
  removeStock(
    ...
    userId = null,
    billId = null,
    movementType = stockMovementEnum.adjust
  ) {
  ...
  async addStockByProductAndWarehouse(productId, warehouseId, quantity, createdBy, billId = null) {
    ...
    if (stock) {
      const previousQuantity = stock.quantity;
      stock = await Stock.findByIdAndUpdate(stock._id, { $inc: { quantity } }, { new: true });
      this._logMovement({
        type: stockMovementEnum.in,
        quantity,
        previousQuantity,
