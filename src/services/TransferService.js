...
class TransferService {
  ...
  async transferStock(productId, fromWarehouseId, toWarehouseId, quantity, userId) {
    ...
    try {
      ...
      if (error.code === 251 || error.message?.includes('Transaction numbers')) {
        return this._transferWithoutTransaction(
          productId,
          fromWarehouseId,
          toWarehouseId,
          quantity,
    ...
    try {
      ...
      if (!fromStock) {
        ...
        const error = new Error(
          stock ? 'Insufficient stock in source warehouse' : 'Stock not found in source warehouse'
        );
        error.code = 4005;
        throw error;
      }
      ...
      this._logTransferMovement(
        populatedResult.fromStock,
        populatedResult.toStock,
        quantity,
        userId
      );
  ...
  async _transferWithoutTransaction(productId, fromWarehouseId, toWarehouseId, quantity, userId) {
    ...
    if (toStock) {
      toStock = await Stock.findByIdAndUpdate(toStock._id, { $inc: { quantity } }, { new: true });
    } else {
      toStock = new Stock({
        productId,
        warehouseId: toWarehouseId,
  ...
  async getProductStockByWarehouse(productId) {
    const stocks = await Stock.find({ productId }).populate('warehouseId').populate('productId');
