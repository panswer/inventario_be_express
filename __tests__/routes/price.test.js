...
describe("Price Routes", () => {
  ...
  beforeAll(async () => {
    ...
    token = jwt.sign({ _id: userId, role: "admin", sessionId: "test-session-price" }, process.env.SERVER_JWT_SESSION_SECRET);
    await global.createTestSession(userId.toString(), "test-session-price");
  });
